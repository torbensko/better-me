...
function DonutChart(
  data: IData[],
  {
    ...
    value = (y) => y.value, // given d in data, returns the (quantitative) value
  ...
  const colorSet =
    colors ||
    d3.schemeSpectral[nameSet.size] ||
    d3.quantize((t) => d3.interpolateSpectral(t * 0.8 + 0.1), nameSet.size);

  // Construct scales.
  console.log(nameSet, colors, nameSet.size, d3.schemeSpectral);
...
export const Medalion: React.FC<IMedalionProps> = ({}) => {
  ...
  useEffect(() => {
    ...
    medalion.current?.appendChild(chart);
  }, [medalion]);

  return <div ref={medalion}>My Medalion</div>;
};
